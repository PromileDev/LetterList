...
<template>
    ...
    <div v-else-if="!loading" class="mx-auto w-11/12 min-h-[65vh] flex flex-col">
        ...
        <div class="mt-10 h-40 flex gap-4">
            ...
            <div @click="showSite(website.name)" href="" class="w-40 h-full border-2 border-light rounded-lg hover:shadow-lg cursor-pointer" v-for="(website) in websites" :key="website.id">
                {{ website.name }}
            </div>
        </div>
    </div>
